...
The original code has been edited so that it only does line plots (no drum plot), has a lowpass and highpass filter, has two more arguments that must be included for it to run, turns red when data exceeds threshold value, and stations turn black when producing unusable data.
